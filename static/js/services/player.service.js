...
export async function loadAndPlay(track) {
    ...
    try {
        ...
        let msg = "Error reproduciendo: " + track.title;
        if (e.name === "NotSupportedError") {
            msg = "Error 403: Link Expirado o Bloqueado por IP.";
            console.warn("Posible bloqueo de YouTube (IP Mismatch).");
        }
...
function updateProgress() {
    ...
    // Fallback if streamed audio has infinite duration (common in chunks)
    if (!Number.isFinite(duration) && state.currentQueue[state.currentIndex]) {
        // API duration is in seconds
    ...
    if (duration && Number.isFinite(duration) && duration > 0) {
        ...
        document.getElementById("tot-time").innerText = formatTime(duration);
    } else {
        document.getElementById("curr-time").innerText = formatTime(audioPlayer.currentTime);
        document.getElementById("tot-time").innerText = "--:--";
    }
}
